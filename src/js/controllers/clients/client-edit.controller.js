...
ClientEditCtrl.$inject = ['Client', 'Type', 'Zone', 'Location', '$window', '$stateParams', '$http', '$state', '$rootScope'];

function ClientEditCtrl(Client, Type, Zone, Location, $window, $stateParams, $http, $state, $rootScope) {
  const vm = this;
  ...
  function cancel() {
    if (!vm.wasLocationDeleted) {
      Location
        .delete({ id: vm.client.indirizzo.id })
        .$promise
        .then(() => {
          vm.results = null;
          vm.random = {};
